...
class GalaxyImageResiduals(Module):
    ...
    def chi_squared(self, x, y):
        x = pyro.deterministic("obs_values", torch.masked_select(x, self.mask))
        y = pyro.deterministic("model_values", torch.masked_select(y, self.mask))
        chi_sqrd = torch.square(y - x) / self.yerr ** 2
        return torch.sum(chi_sqrd)
...
class GalaxyLogLikelihood(Module):
    ...
    def chi_squared_residuals(self, translation, tau, obs):
        ...
        translation = pyro.deterministic("expanded_translation",
        ...
        corr_trans_model = pyro.deterministic("corr_trans_model",
...
def model(obs):
    ...
    chi_squared = log_likelihood_fun(translation, tau, obs)
    print(chi_squared, torch.log(chi_squared))
    pyro.factor("log_likelihood_fun", torch.log(chi_squared))

    # using Delta atm, consider looking into empirical distributions
    # when using empirical potential to weight instead of mask
    return torch.log(chi_squared) # pyro.sample("obs", dist.Normal(_model, 3.0e-3).to_event(1), obs=_obs) # .to_event(1) # torch.sum(s) # s # pyro.sample("obs", dist.Delta(_model).to_event(1), obs=_obs)
...
if __name__ == "__main__":
    ...
    for i in range(len(synthetic_obs)):
        im = axes[i].imshow(synthetic_obs[i].cpu().numpy(), origin="lower", vmin=0.)
    ...
    pyro.render_model(guide, model_args=(synthetic_obs, ),
